...
// Helper Voxel component with cartoon outline (same toon technique as DuckDinoModel)
function Voxel({
  position = [0, 0, 0],
  rotation = [0, 0, 0],
  args,
...
export function RabbitDinoModel({ animState, previewMode = false, skinConfig }: RabbitDinoProps) {
  ...
  const baseColor = skinConfig.baseColor; // Blue
  const bellyColor = skinConfig.spotsColor; // White
  const earTrimColor = skinConfig.spikesColor; // Darker blue
  const collarColor = skinConfig.collarColor; // Pink

  // Shared blinking material (arms/legs only, matching DuckDinoModel's pattern)
  ...
  useFrame((state, delta) => {
    ...
    if (previewMode) {
      ...
      if (earLeftRef.current) earLeftRef.current.rotation.x = 0;
      if (earRightRef.current) earRightRef.current.rotation.x = 0;
      return;
    }
    ...
    if (status === 'gameover') {
      ...
      if (earLeftRef.current) earLeftRef.current.rotation.x = -1.0;
      if (earRightRef.current) earRightRef.current.rotation.x = -1.0;
      ...
      // Ears perk up / lay back with jumps, plus a gentle idle sway
      const time = state.clock.getElapsedTime();
      if (earLeftRef.current && earRightRef.current) {
        if (!isGrounded && current.velocity > 0) {
          // Ears sweep back on takeoff
          earLeftRef.current.rotation.x = THREE.MathUtils.lerp(earLeftRef.current.rotation.x, -0.6, 0.25);
          earRightRef.current.rotation.x = THREE.MathUtils.lerp(earRightRef.current.rotation.x, -0.6, 0.25);
        } else {
          const sway = Math.sin(time * 3) * 0.05;
          earLeftRef.current.rotation.x = THREE.MathUtils.lerp(earLeftRef.current.rotation.x, sway, 0.15);
          earRightRef.current.rotation.x = THREE.MathUtils.lerp(earRightRef.current.rotation.x, sway, 0.15);
        }
      }
    }
    ...
    // 5. Eye spring blinking animation
    const time2 = state.clock.getElapsedTime();
    const blinkCycle = time2 % 4.0;
  ...
  return (
    <group ref={parentGroupRef}>
      ...
      <Voxel position={[-0.1, 1.15, 0]} args={[1.3, 1.1, 0.9]} color={baseColor} isGhost={isGhost}>
        {/* White Belly Patch */}
        <Voxel position={[0.2, -0.2, 0]} args={[0.85, 0.55, 0.7]} color={bellyColor} isGhost={isGhost} outline={false} />

        {/* Collar / Bow */}
        <Voxel position={[0.65, 0.1, 0]} args={[0.08, 0.2, 0.4]} color={collarColor} isGhost={isGhost} />

        {/* Fluffy round bunny tail */}
        <Voxel position={[-0.78, 0.3, 0]} args={[0.32, 0.32, 0.32]} color={bellyColor} isGhost={isGhost} />
      </Voxel>
      ...
      <group ref={headRef} position={[0.4, 1.8, 0]}>
        ...
        <Voxel position={[0, 0.4, 0]} args={[1.1, 1.1, 1.1]} color={baseColor} isGhost={isGhost}>
          ...
          {/* Left Rabbit Ear */}
          <group ref={earLeftRef} position={[-0.15, 0.55, 0.22]}>
            <Voxel position={[0, 0.55, 0]} args={[0.28, 1.1, 0.16]} color={baseColor} isGhost={isGhost}>
              <Voxel position={[0, 0, 0.01]} args={[0.14, 0.85, 0.05]} color={bellyColor} isGhost={isGhost} outline={false} />
            </Voxel>
          </group>

          {/* Right Rabbit Ear */}
          <group ref={earRightRef} position={[-0.15, 0.55, -0.22]}>
            <Voxel position={[0, 0.55, 0]} args={[0.28, 1.1, 0.16]} color={baseColor} isGhost={isGhost}>
              <Voxel position={[0, 0, -0.01]} args={[0.14, 0.85, 0.05]} color={bellyColor} isGhost={isGhost} outline={false} />
            </Voxel>
          </group>
          ...
          <group ref={leftEyeGroupRef} position={[0.35, 0.15, 0.56]}>
            ...
            <Voxel position={[0.05, 0, 0.01]} args={[0.16, 0.16, 0.05]} color={earTrimColor} isGhost={isGhost} outline={false} />
          </group>
          ...
          {/* Small snout with white muzzle patch */}
          <Voxel position={[0.62, -0.2, 0]} args={[0.4, 0.4, 0.65]} color={baseColor} isGhost={isGhost} />
          <Voxel position={[0.82, -0.28, 0]} args={[0.18, 0.28, 0.4]} color={bellyColor} isGhost={isGhost} outline={false} />

          {/* Two big buck teeth */}
          <Voxel position={[0.88, -0.48, 0.1]} args={[0.12, 0.24, 0.1]} color="#ffffff" isGhost={isGhost} />
          <Voxel position={[0.88, -0.48, -0.1]} args={[0.12, 0.24, 0.1]} color="#ffffff" isGhost={isGhost} />
        </Voxel>
      </group>
